...
from autopipe.steps.explainability import PermutationImportanceStep
...
class TestEndToEndMLPipeline:
    ...
    def test_complete_classification_pipeline(self, sample_classification_data, temp_dir):
        ...
        splitter = DataSplitterStep(
            name="data_split", train_size=0.7, val_size=0.15, test_size=0.15, random_state=42
        )
...
class TestCrossValidationIntegration:
    ...
    def test_cv_with_nested_pipeline(self):
        ...
        cv_step = CrossValidationStep(
            name="cv", strategy="stratified", n_splits=3, metrics=["accuracy", "f1"]
        )

        # Run CV
        results = cv_step.run(
            model_builder=lambda: RandomForestClassifier(n_estimators=10, random_state=42), X=X, y=y
        )
...
class TestExplainabilityIntegration:
    ...
    def test_permutation_importance_with_model(self):
        ...
        perm_imp_step = PermutationImportanceStep(name="perm_importance", n_repeats=5)

        result = perm_imp_step.run(
            model=model, X=X, y=y, feature_names=[f"feat_{i}" for i in range(10)]
        )
...
class TestFeatureEngineeringIntegration:
    ...
    def test_feature_engineering_pipeline(self):
        ...
        feature_step = FeatureSelectionStep(name="feature_selection", method="variance_threshold")
...
class TestDriftDetectionIntegration:
    ...
    def test_drift_detection_no_drift(self):
        ...
        detector = StatisticalDriftDetectorStep(name="drift_detector", method="ks", threshold=0.05)

        result = detector.run(reference_data=ref_data, current_data=cur_data)
