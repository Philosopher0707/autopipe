...
async def test_sidebar_counts(auth_client: AsyncClient):
    """GET /dashboard/counts returns sidebar badge counts."""
    resp = await auth_client.get("/api/v1/dashboard/counts")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_health_check(auth_client: AsyncClient):
    ...
    resp = await auth_client.get("/api/v1/dashboard/health")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_activity_feed(auth_client: AsyncClient):
