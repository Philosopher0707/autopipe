...
async def list_runs(
    ...
    if search:
        ...
        query = query.where(Run.id.ilike(search_filter) | Pipeline.name.ilike(search_filter))
    ...
    # Get pipeline names
    pipeline_ids = {r.pipeline_id for r in runs}
    pipeline_result = await db.execute(
        select(Pipeline.id, Pipeline.name).where(Pipeline.id.in_(pipeline_ids))
    )
    pipeline_names = dict(pipeline_result.fetchall())
    ...
    if project_ids:
        ...
        project_names = dict(project_result.fetchall())
...
async def get_run(
    ...
    result = await db.execute(select(Run).where(Run.id == run_id).options(selectinload(Run.steps)))
...
async def get_run_steps(
    ...
    result = await db.execute(select(Step).where(Step.run_id == run_id).order_by(Step.order_index))
    steps = result.scalars().all()
...
async def add_step_log(
    ...
    result = await db.execute(select(Step).where(Step.id == step_id).where(Step.run_id == run_id))
...
async def compare_multiple_runs(
    ...
    for key in sorted(all_param_keys):
        ...
        for r in runs_ordered:
            ...
            try:
                vals_seen.add(
                    json.dumps(val, sort_keys=True) if isinstance(val, (dict, list)) else str(val)
                )
