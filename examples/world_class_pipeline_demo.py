...
def example_1_cv_and_training():
    ...
    print(f"  Mean Train Score: {cv_results['mean_train_score']:.4f} ± {cv_results['std_train_score']:.4f}")
    print(f"  Mean Val Score: {cv_results['mean_val_score']:.4f} ± {cv_results['std_val_score']:.4f}")
...
def example_4_model_registry():
    ...
    models = [
        ...
        ("RandomForest_v2", RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42)),
...
def example_5_explainability():
    ...
    if 'aggregated_ranks' in results:
        ...
        for i, (feat, rank) in enumerate(list(results['aggregated_ranks'].items())[:5]):
...
def example_6_drift_detection():
    ...
    for i in range(5):
        df_current[f'feature_{i}'] = df_current[f'feature_{i}'] + np.random.normal(5, 1, len(df_current))
